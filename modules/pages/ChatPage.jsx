...
const ChatPage = () => {
  ...
  return (
    <div id="tour-terminal" className="flex flex-col h-full glass-panel rounded-xl border border-border p-4 pb-20 md:pb-8 md:p-8 relative overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-6 flex-shrink-0">
        <div className="flex flex-col">
          <nav aria-label="breadcrumb" className="flex items-center space-x-2 text-sm text-textMuted mb-0.5">
            ...
            <span className="flex items-center text-lg font-medium text-textPrimary">Asystent AI</span>
          </nav>
          <p className="font-sans text-xs text-textMuted mt-0.5">Inteligentny interfejs konwersacyjny</p>
        </div>
        <div className="flex items-center gap-4">
          <button 
            onClick={() => setShowModels(!showModels)}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-sm font-sans font-medium transition-colors ${showModels ? 'bg-accentPrimary border-accentPrimary text-black' : 'border-border text-textPrimary hover:bg-surface'}`}
          >
      ...
      {showModels && (
        <div className="absolute top-20 right-4 md:right-8 w-full max-w-[400px] h-[350px] z-50 shadow-[0_0_30px_rgba(var(--color-accent-primary-hex),0.2)] rounded-xl opacity-0 animate-soft-enter" style={{ animationDelay: '50ms' }}>
          <button 
