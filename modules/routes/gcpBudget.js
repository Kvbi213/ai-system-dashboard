...
router.get('/config', (req, res) => {
  const gcpProjectId = process.env.GCP_PROJECT_ID || 'omnidash-509607';
  const hostingDomain = `${FIREBASE_PROJECT_ID || process.env.FIREBASE_PROJECT_ID || 'void-potato-7721'}.web.app`;
  res.json({
    projectId: gcpProjectId,
    projectName: 'OmniDash (Google Cloud Console)',
    topicName: `projects/${gcpProjectId}/topics/omni-budget-alerts`,
    subscriptionName: `projects/${gcpProjectId}/subscriptions/omni-budget-push`,
    pushEndpointUrl: `https://${hostingDomain}/api/gcp/budget-webhook`,
    billingConsoleUrl: `https://console.cloud.google.com/billing/budgets?project=${gcpProjectId}`,
    pubsubConsoleUrl: `https://console.cloud.google.com/cloudpubsub/topic/list?project=${gcpProjectId}`,
    setupSteps: [
      `1. Włącz interfejs Cloud Pub/Sub: gcloud services enable pubsub.googleapis.com --project=${gcpProjectId}`,
      `2. Utwórz temat Pub/Sub: gcloud pubsub topics create omni-budget-alerts --project=${gcpProjectId}`,
      `3. Utwórz subskrypcję Push: gcloud pubsub subscriptions create omni-budget-push --topic=omni-budget-alerts --push-endpoint=https://${hostingDomain}/api/gcp/budget-webhook --project=${gcpProjectId}`,
