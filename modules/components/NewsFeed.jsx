...
function timeAgo(dateStr, t) {
  ...
  if (diff < 86400) return `${Math.floor(diff / 3600)}${t('timeAgoHours')}`;
  return `${Math.floor(diff / 86400)}${t('timeAgoDays')}`;
}
  ...
  return (
    <div className="glass-panel h-full rounded-xl p-4 flex flex-col">
      <div className="flex items-center gap-2 mb-3 border-b border-border pb-2.5 flex-shrink-0">
        ...
        <h2 className="font-mono text-xs uppercase tracking-widest text-textMuted flex-1">{t('newsFeedTitle')}</h2>
          ...
          logs.map(log => (
            <div key={log.id} className="group border-l-2 border-border hover:border-accentSecondary pl-3 transition-colors">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] font-mono text-textMuted">{timeAgo(log.created_at, t)}</span>
              </div>
